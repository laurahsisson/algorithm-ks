...
class Line(_BBox):
    ...
    def setDash(self,option):
        self._reconfig("dash",option)
...
def pointInRect(point, rect):
    ...
    return point.x >= p1.x and point.y >= p1.y and point.x <= p2.x and point.y <= p2.y

def linesIntersect(L1,L2):
    def isCollinear():
        return pointInRect(L1.p1,r2) or pointInRect(L1.p2,r2) or pointInRect(L2.p1,r1) or pointInRect(L2.p2,r1)
    r1 = Rectangle(Point(min(L1.p1.x,L1.p2.x),min(L1.p1.y,L1.p2.y)),Point(max(L1.p1.x,L1.p2.x),max(L1.p1.y,L1.p2.y)))
    r2 = Rectangle(Point(min(L2.p1.x,L2.p2.x),min(L2.p1.y,L2.p2.y)),Point(max(L2.p1.x,L2.p2.x),max(L2.p1.y,L2.p2.y)))
    ...
    if m1==None and m2==None:
    ...
    elif m1 != None and m2 != None and isclose(m1,m2):
        if isCollinear():
            # True collinear
            return isclose(b1,b2)
        else:
            # Parallel
            return False
    else:
        ...
        if m1 == None:
            ...
            yHat = xHat * m1 + b1
        xy = Point(xHat,yHat)
        if pointInRect(xy,r1) and pointInRect(xy,r2):
            # Intersection is valid!
            return True
        else:
            # Intersection occurs outside of bounds of line segments
            return False
